...
% 2,3,5,8,6,9,4,7 - sampled from a multi-varaite GMM (number of components = 2). 
% Different models deoending on x13 (x4, x7 polarity)
...
pd1 = gmdistribution.fit(m(ind,[2,3,5,8,6,9,4,7]),2);
pd2 = gmdistribution.fit(m(~ind,[2,3,5,8,6,9,4,7]),2);
...
for i = 1:numsample,
    ...
    %x11(1,i) = normrnd(mean(m(:,11)),-0.0174*x3(1,i).^4 +0.0515*x3(1,i).^3 +  0.6114 *x3(1,i).^2 +  1.2865*x3(1,i) + 1.0015);
...
for i = 1:numsample,
    ...
    if m(i,13) ==1,
        lprob(i,2) = lprob(i,2) + log(pdf(pd1,[m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)]));
        %log(pdf(pd1,[m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)]));
    else
        lprob(i,2) = lprob(i,2) + log(pdf(pd2,[m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)]));
        %log(pdf(pd2,[m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)]));
    end
    
    % add log probabilities for 1,12
    lprob(i,2) = lprob(i,2) + log(normpdf(m(i,1)-cos((m(i,6)-m(i,9))),mean(m(:,1)-cos(m(:,6)-m(:,9))),std(m(:,1)-cos(m(:,6)-m(:,9)))));
    %log(normpdf(m(i,1)-cos(2*(m(i,6))),mean(m(:,1)-cos(2*m(:,6))),std(m(:,1)-cos(2*m(:,6)))))
    lprob(i,2) = lprob(i,2) + log(normpdf(m(i,12)-sin((m(i,6)-m(i,9))),mean(m(:,12)-sin(m(:,6)-m(:,9))),std(m(:,12)-sin(m(:,6)-m(:,9)))));
...
%     lprob(i,2) = lprob(i,2) + log(normpdf(m(i,11),mean(m(:,11)),0.027*m(i,3).^4 +0.2*m(i,3).^3 + 0.52*m(i,3).^2 + 0.99*m(i,3) +1));
%     t1(i,1) = 0.03*m(i,3).^4 +0.18*m(i,3).^3 + 0.5*m(i,3).^2 + 0.97*m(i,3) +1;
%     t1(i,2) = 0.028*m(i,3).^4 +0.15*m(i,3).^3 + 0.52*m(i,3).^2 + 0.99*m(i,3) +0.99;
%     t1(i,3) = m(i,3);
