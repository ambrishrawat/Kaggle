% skewed Normal distribution for x2
...
for i = 1:numsample,
    ...
    if m(i,13) == 1
        lprob(i,2) = lprob(i,2) + log(mvnpdf([m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)],mean11(2:8),cov11(2:8,2:8))); 
        %log(mvnpdf([m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)],mean11,cov11))
    else
        lprob(i,2) = lprob(i,2) + log(mvnpdf([m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)],mean22(2:8),cov22(2:8,2:8)));
        %log(mvnpdf([m(i,2),m(i,3),m(i,5),m(i,8),m(i,6),m(i,9),m(i,4),m(i,7)],mean22,cov22))
    end
    moments = {mean(m(:,2)),std(m(:,2)),skewness(m(:,2)),kurtosis(m(:,2))};
    lprob(i,2) = lprob(i,2) + log(pearspdf(m(i,2),moments{:}));
